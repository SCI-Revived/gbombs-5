...
ENT.Model                            =  "models/thedoctor/avt_virus.mdl"                      
ENT.Effect                           =  "vx_gas"                  
ENT.EffectAir                        =  "vx_gas_ground"                   
ENT.EffectWater                      =  "water_medium"
ENT.ExplosionSound                   =  "gbombs_5/explosions/nuclear/blast_wave.mp3"
ENT.ArmSound                         =  ""            
ENT.ActivationSound                  =  ""     
...
function ENT:Explode()
	...
	for k, v in pairs(gb5FastSphereSearch(self:GetPos(),200)) do
		...
		if v:GetClass()=="gb5_chemical_tvirus_entity" then
		...
		if v:IsNPC() and table.HasValue(ZombieList,v:GetClass()) then
			if math.random(1,4)==4 then
				...
				ent:SetPos(v:GetPos())
				ent:Spawn() 
				ent:EmitSound("gbombs_5/explosions/special/zomb.wav")
				ent:StopParticles()
				v:Remove()		
			end
		end
