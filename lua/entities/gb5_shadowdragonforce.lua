...
function ENT:Think()
	if (CLIENT) then
		function Draw_Mod()
			local tab = {}
			tab[ "$pp_colour_addr" ] = 0 - LocalPlayer():GetNWInt("color", 0)/3
			tab[ "$pp_colour_addg" ] = 0 - LocalPlayer():GetNWInt("color", 0)/3
			tab[ "$pp_colour_addb" ] = 0 - LocalPlayer():GetNWInt("color", 0)/4
	...
	if self.radowner:IsValid() then 
		if self.initialised==nil then
			...
			self.radowner:EmitSound("dragonforce", 100, 100)
		end
