...
function ENT:Explode()
	...
	local Shockwave = gb5BeginShockwave() do
		...
		Shockwave.Sound              = "gbombs_5/explosions/special/repulsion_bomb.wav"
	...
	if(self:WaterLevel() >= 1) then
		...
		if trace.HitWorld then
			ParticleEffect("repulsion_ground",pos,angle_zero,nil)	
			timer.Simple(0.1, function()
				if not self:IsValid() then return end 
					ParticleEffect("",trace.HitPos,angle_zero,nil)	
			end)	
		else 
			ParticleEffect("repulsion_air",pos,angle_zero,nil) 
